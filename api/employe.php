...
function register_emp($conn){
    extract($_POST);
    $data = array();
    $query = "CALL employee_sp('','$name','$gender','$Phone','$Address','$salary')";
...
function update_emp($conn){
    extract($_POST);
    $data = array();
    $query = "CALL employee_sp('$update_id','$name','$gender','$Phone','$Address','$salary')";
    $result = $conn->query($query);
    if($result){
       $row = $result->fetch_assoc(); 
